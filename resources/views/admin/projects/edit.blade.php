            ...
            <div class="mt-6">
                            ...
                            @foreach($project->gallery as $imagePath)
                                <div class="relative group">
                                    ...
                                    <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                                        <span class="text-white text-xs">{{ basename($imagePath) }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <p class="text-sm text-gray-500 mt-2">{{ count($project->gallery) }} image(s) in gallery</p>
                    </div>
                @endif
                ...
                <div>
                    ...
                    <input type="file" name="gallery_files[]" id="gallery_files" accept="image/*" multiple
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <p class="text-sm text-gray-500 mt-1">Select multiple images to replace the current gallery. Leave empty to keep current gallery. Supported formats: JPEG, PNG, JPG, GIF, WebP. Max size per file: 25MB</p>
                    @error('gallery_files')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    @error('gallery_files.*')
